...
const TextInputField = ({data, jsonKey}) => {
  ...
  return (   
    <FormControl isRequired={data.validate.required} marginY={3} isInvalid={checkValid()}>
        <FormLabel>{data.label} {data.description &&  <Tooltip label={data.description} fontSize='sm'>
  <InfoIcon />
</Tooltip>}</FormLabel>
        <Input placeholder={data.placeholder} onChange={handleChange} readOnly={data.validate.immutable}/>
    </FormControl>
);
};
